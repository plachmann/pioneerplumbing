( function ( $, TM_PageBuilder, Backbone ) {
	...
	TM_PageBuilder.AppView = Backbone.View.extend( {
		...
		initialize : function() {
			...
			this.listenTo( TM_PageBuilder_Events, 'tm-pb-content-updated', this.updateYoastContent );
		...
		updateYoastContent : function() {
			...
			var content = tm_pb_get_content( 'content', true );
			...
			$.ajax( {
				...
				data: {
					action : 'tm_pb_execute_content_shortcodes',
					tm_admin_load_nonce : tm_pb_options.tm_admin_load_nonce,
					tm_pb_unprocessed_data : content
				},
				success: function( data ) {
					tm_pb_processed_yoast_content = data;
