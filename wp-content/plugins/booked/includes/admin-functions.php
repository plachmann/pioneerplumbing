...
function booked_render_single_timeslot_form($timeslot_intervals,$type = false){
	...
	echo '<form id="single-timeslot-form" action="" method="post">';
		...
		echo booked_render_text_field('title', __('Title (optional)','booked'));
...
function booked_render_time_select($select_name,$interval,$placeholder,$single = false){
	...
	$html .= '<option value="">'.$placeholder.'</option>';
		...
		$html .= '<option value="2400">'.date_i18n($time_format,strtotime('2014-01-01 24:00')).' ('.__('night','booked').')</option>';
	$html .= '</select>';
...
function booked_admin_calendar_date_loop($date,$time_format,$date_format,$calendar_id = false,$tab_title,$tabbed = false,$calendars = false){
	...
	*/
	...
	elseif (isset($booked_defaults[$day_name]) && !empty($booked_defaults[$day_name])):
		...
		$todays_defaults_details = $booked_defaults[$day_name.'-details'];
	else :
		$todays_defaults = false;
		$todays_defaults_details = false;
	endif;
		...
		foreach($todays_defaults as $timeslot => $count):
			...
			*/
			...
			echo '<div class="timeslot bookedClearFix'.($title ? ' has-title ' : '').'">';
				...
				echo '</span>';
				echo '<span class="timeslot-people"><button'.(!$available ? ' disabled' : '').' data-timeslot="'.$timeslot.'" data-title="'.esc_attr($title).'" data-date="'.$date.'"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').' class="new-appt button"'.(!$spots_available ? ' disabled' : '').'>'.__('New Appointment','booked').' ...</button></span>';
			echo '</div>';
					...
					if ($look_for_subs):
						...
						if ($field_type == 'single-checkbox'):
							...
							?><li class="ui-state-default"><i class="sub-handle fa fa-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this checkbox...','booked'); ?>" />
						...
						elseif ($field_type == 'single-radio-button'):
							...
							?><li class="ui-state-default"><i class="sub-handle fa fa-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this radio button...','booked'); ?>" />
								<span class="cf-delete"><i class="fa fa-close"></i></span>
							</li><?php
						...
						elseif ($field_type == 'single-drop-down'):
							...
							?><li class="ui-state-default"><i class="sub-handle fa fa-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this option...','booked'); ?>" />
								<span class="cf-delete"><i class="fa fa-close"></i></span>
							</li><?php
					...
					switch($field_type):
						...
						case 'single-line-text-label' :
							...
							?><li class="ui-state-default"><i class="main-handle fa fa-bars"></i>
								...
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this field...','booked'); ?>" />
								<span class="cf-delete"><i class="fa fa-close"></i></span>
							</li><?php
						...
						case 'checkboxes-label' :
							...
							?><li class="ui-state-default"><i class="main-handle fa fa-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php _e('Required Field','booked'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this checkbox group...','booked'); ?>" />
						...
						case 'radio-buttons-label' :
							...
							?><li class="ui-state-default"><i class="main-handle fa fa-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php _e('Required Field','booked'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php _e('Enter a label for this radio button group...','booked'); ?>" />
