...
if ($appt_is_available):
	...
	if ($customer_type == 'guest'):
		...
		if ($email && is_email($email) && $name || !$email && !$email_required && $name):
			...
			if ($email_content && $email_subject):
				...
				$replacements = array($name,date_i18n($date_format,$timestamp),$timeslotText,$cf_meta_value,$calendar_name,$email,$title);
			...
			if ($email && !is_email($email)):
				$errors[] = __('The email address you have entered doesn\'t appear to be valid.','booked');
			elseif ($email_required):
				$errors[] = __('Your name and a valid email address are required to book an appointment.','booked');
			else:
				$errors[] = __('Your name is required to book an appointment.','booked');
			endif;
	...
	elseif ($customer_type == 'new'):
		...
		if (empty($errors)):
			...
			if ($email_content && $email_subject):
				...
				$replacements = array($name,$email,$username,$password);
				$email_content = htmlentities(str_replace($tokens,$replacements,$email_content), ENT_QUOTES | ENT_IGNORE, "UTF-8");
				$email_content = html_entity_decode($email_content, ENT_QUOTES | ENT_IGNORE, "UTF-8");
				$email_subject = str_replace($tokens,$replacements,$email_subject);
				booked_mailer( $email, $email_subject, $email_content );
	...
	echo 'error###'.__('Sorry, someone just booked this appointment before you could. Please choose a different booking time.','booked');
