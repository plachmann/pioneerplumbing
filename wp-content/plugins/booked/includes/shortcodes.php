...
class BookedShortcodes {
	...
	public function booked_registration_redirect(){
		...
		if ( get_option('users_can_register') && isset($_POST['booked_reg_submit'] ) ) {
	        ...
	        global $registration_complete, $booked_reg_errors, $username, $name, $email, $password;

			$name 			= isset($_POST['booked_reg_name']) ? esc_attr($_POST['booked_reg_name']) : '';
	        $username 		= sanitize_user($name);
	        $password 		= isset($_POST['booked_reg_password']) ? $_POST['booked_reg_password'] : '';
	        $email      	= isset($_POST['booked_reg_email']) ? sanitize_email(esc_attr($_POST['booked_reg_email'])) : '';
	...
	public function booked_calendar_shortcode($atts, $content = null){
		...
		if ($atts['members-only'] && is_user_logged_in() || !$atts['members-only']):
			...
			echo '<div class="booked-calendar-shortcode-wrap">';
				...
				if (get_option('booked_hide_default_calendar') && !$atts['calendar'] && !empty($calendars)):
				...
				echo '<div class="booked-profile-appt-list">';
					...
					foreach($appointments_array as $appt):
						...
						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.$appt['post_id'].'">';
							...
							echo (!empty($appt['calendar_id']) ? '<i class="fa fa-calendar-o"></i><strong>'.__('Calendar','booked').':</strong> '.$appt['calendar_id'][0]->name.'<br>' : '');
							...
							if (!$historic):
								if ($appt_date_time >= $date_to_compare):
									...
									if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) && !get_option('booked_dont_allow_user_cancellations',false) ) { if ( $appt_date_time >= $date_to_compare ) { echo '<a href="#" data-appt-id="'.$appt['post_id'].'" class="cancel">'.__('Cancel Appointment','booked').'</a>'; } }
			...
			?><div id="booked-profile-page">
				...
				<div id="booked-page-form">
					...
					<ul class="booked-tabs login bookedClearFix">
						<li<?php if ( !isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#login"><i class="fa fa-lock"></i><?php _e('Sign In','booked'); ?></a></li>
						<?php if (get_option('users_can_register')): ?><li<?php if ( isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#register"><i class="fa fa-pencil"></i><?php _e('Register','booked'); ?></a></li><?php endif; ?>
						<li><a href="#forgot"><i class="fa fa-life-ring"></i><?php _e('Forgot Password','booked'); ?></a></li>
					</ul>
					...
					<div id="profile-login" class="booked-tab-content">
						...
						<div class="booked-form-wrap bookedClearFix">
							...
							<?php echo wp_login_form( array( 'echo' => false, 'redirect' => get_the_permalink($login_redirect), 'label_username' => __( 'Email','booked' ) ) ); ?>
						</div>
					</div>
					...
					<div id="profile-register" class="booked-tab-content">
						<div class="booked-form-wrap bookedClearFix">
							...
							booked_registration_form($name,$email,$password);
					...
					<div id="profile-forgot" class="booked-tab-content">
						<div class="booked-form-wrap bookedClearFix">
							<div class="booked-custom-error"><?php _e('An email address is required to reset your password.','booked'); ?></div>
							<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
								<p class="username">
									<label for="user_login"><?php _e('What is your email address?','booked'); ?></label>
