<div class="booked-settings-wrap wrap"><?php
	...
	if (!current_user_can('manage_booked_options') && $booked_none_assigned):
		...
		<div class="booked-pending-cap bookedClearFix">
			<button class="delete-past button"><?php _e('Delete Passed Appointments','booked'); ?></button>
			<button style="float:right; margin-left:10px;" class="approve-all button button-primary"><?php _e('Approve All','booked'); ?></button>
			<button style="float:right;" class="delete-all button"><?php _e('Delete All','booked'); ?></button>
		</div><?php
		...
		echo '<div class="booked-pending-appt-list">';
			...
			*/
				...
				foreach($appointments_array as $appt):
					...
					echo '<div class="pending-appt bookedClearFix'.($late_date > $date_to_compare ? ' passed' : '').'" data-appt-id="'.$appt['post_id'].'">';
						...
						echo '<span class="appt-block" data-appt-id="'.$appt['post_id'].'">';
							...
							echo '<button class="delete button">'.__('Delete','booked').'</button>';
		
							echo $user_display;
							
							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . __('This appointment has passed.','booked') . '</span><br>'; endif;
							if ($calendar_owner_name): echo '<i class="fa fa-user"></i>&nbsp;&nbsp;'.sprintf(__('Assigned to %s','booked'),$calendar_owner_name).'<br>'; endif;
			...
			echo '<div class="pending-appt'.(!empty($appointments_array) ? ' no-pending-message' : '').'">';
				echo '<p style="text-align:center; margin:3em 0 2em">'.__('There are no pending appointments.','booked').'</p>';
			echo '</div>';
