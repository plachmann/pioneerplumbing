...
if ( ! class_exists( '__Tm_Widget_Area' ) ) {
	...
	class __Tm_Widget_Area {
		...
		function __construct() {
			...
			add_action( 'plumberpro_render_widget_area', array( $this, 'render' ) );
		}
		...
		public function render( $area_id ) {
			...
			$area_id        = apply_filters( 'plumberpro_rendering_current_widget_area', $area_id );
			...
			$classes = apply_filters( 'plumberpro_widget_area_classes', $classes, $area_id );
	...
	function plumberpro_widget_area() {
		return __Tm_Widget_Area::get_instance();
	}

	plumberpro_widget_area();
}
