 ...
 * @return void
 */
function plumberpro_main_menu() { ?>
	<nav id="site-navigation" class="main-navigation" role="navigation">
		<button class="menu-toggle" aria-controls="main-menu" aria-expanded="false"><i class="material-icons">subject</i></button>
		<?php
			$args = apply_filters( 'plumberpro_main_menu_args', array(
				'theme_location'   => 'main',
				'container'        => '',
				'menu_id'          => 'main-menu',
				'fallback_cb'      => 'plumberpro_set_nav_menu',
				'fallback_message' => esc_html__( 'Set main menu', 'plumberpro' ),
			) );
 ...
 * @return void
 */
function plumberpro_footer_menu() { ?>
	<nav id="footer-navigation" class="footer-menu" role="navigation">
	<?php
		$args = apply_filters( 'plumberpro_footer_menu_args', array(
			...
			'fallback_message' => esc_html__( 'Set footer menu', 'plumberpro' ),
		) );
 ...
 * @return string
 */
function plumberpro_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'plumberpro_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'plumberpro_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'plumberpro' ),
	) );
 ...
 * @return void
 */
function plumberpro_set_nav_menu( $args ) {

	if ( ! current_user_can( 'edit_theme_options' ) ) {
		return null;
	}

	$format = '<div class="set-menu %3$s"><a href="%2$s" target="_Plumberpro" class="set-menu_link">%1$s</a></div>';
