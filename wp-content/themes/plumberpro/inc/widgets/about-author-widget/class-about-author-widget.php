...
class __Tm_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'plumberpro widget-about-author';
		$this->widget_description = esc_html__( 'Display an information about selected user.', 'plumberpro' );
		$this->widget_id          = 'plumberpro_widget_about_author';
		$this->widget_name        = esc_html__( 'About Author', 'plumberpro' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'About Author', 'plumberpro' ),
			...
			'user_id' => array(
				...
				'options'          => false,
				'label'            => esc_html__( 'Select user to show', 'plumberpro' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'plumberpro' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => esc_html__( 'Select image', 'plumberpro' ),
				'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'plumberpro' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Link (leave empty to hide)', 'plumberpro' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Read More', 'plumberpro' ),
				'label' => esc_html__( 'Link label', 'plumberpro' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => __( 'Select a user', 'plumberpro' ) );
...
add_action( 'widgets_init', 'plumberpro_register_about_author_widgets' );
function plumberpro_register_about_author_widgets() {
